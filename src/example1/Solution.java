...
class Meeting {
    ...
    public boolean equals(Object o) {
        ...
        if (com.begin <= this.begin
                && this.end <= com.end) {
            return true;
        }

        var compareBegin = this.begin <= com.begin && com.begin <= this.end;
        var compareEnd = this.begin <= com.end && com.end <= this.end;
        return compareBegin && compareEnd;
    }
...
//        System.out.println(new Solution().isPalindrome(16121));
...
//        new Solution().recoverTree(errorTree);
//        new Solution().printTreeIteration(errorTree);
...
//                )
//        );
//        var iterator = new BSTIterator(testTree);
...
//        System.out.println(new Solution().letterCombinations("2349"));
//        System.out.println(new Solution().isValid("{{}}()[][[()]]"));
//        System.out.println(new Solution().inorderTraversal(testTree));
...
//        var result = new Solution().removeElement(array, 2);
//        System.out.println(result);
//        var array = new int [] {9,8,9};
//        var result = new Solution().plusOne(array);
//        System.out.println(result);
...
////        var url = "http://www.leetcode.com/faq/?id=10";
//        var url = "ftp://174.123.452.34/directory/file";
//        var encode = codec.encode(url);
//        System.out.println("encode=" + encode);
//        var decode = codec.decode(encode);
//        System.out.println("decode=" + decode);

//        System.out.println(new Solution().strStr("hello", "ll"));
//        System.out.println(new Solution().rearrangeArray(new int[] {3,1,-2,-5,2,-4}));
//
...
//        system.getAverageTime("Leyton","Paradise");
...
//        [1,2,2,1], nums2 = [2,2]
//        System.out.println(new Solution().intersection(new int[] {4, 9 ,5}, new int[] {9,4,9,8,4}));
...
//        System.out.println(new Solution().sumOfLeftLeaves(testTree));

//        System.out.println(new Solution()
//                .maxSubArray(new int[] {-2,1,-3,4,-1,2,1,-5,4}));
//        System.out.println(new Solution().maxSubArray(new int[] {-2,1,-3,4,-1,2,1,-5,4}));
        //[1,1,1]
        var testList1 = new ListNode(
                1,
                new ListNode(
                        1,
                        new ListNode(1)
...
//        new Solution().deleteDuplicates(testList1);

//        [[0,30],[5,10],[15,20]]
//        System.out.println(new Solution().canAttendMeetings(new int[][] {{19,20}, {1,10}, {5,14}}));
//        System.out.println(new Solution().minMeetingRooms(new int[][] {{4,18},{1,35},{12,45},{25,46},{22,27}}));
        ...
        //System.out.println(new Solution().numberOfCombinations("3270"));
        //[[0,3],[1,2],[0,2]]
        var pairs = new ArrayList<List<Integer>>();
        pairs.add(List.of(0, 3));
        pairs.add(List.of(1, 2));
        pairs.add(List.of(0, 2));
//        pairs.add(List.of(1, 4));
//        pairs.add(List.of(2, 3));
//        pairs.add(List.of(5, 7));
    ...
    public String addBinary(String a, String b) {
        var numA = new ArrayList<Integer>(Arrays.stream(a.split(""))
                .mapToInt(Integer::parseInt)
                .boxed()
                .toList());
        Collections.reverse(numA);

        var numB = new ArrayList<Integer>(Arrays.stream(b.split(""))
                .mapToInt(Integer::parseInt)
                .boxed()
                .toList());
        ...
        var extra = 0;
        while (hasNext) {
            var aDigit = numA.size() > index ? numA.get(index) : 0;
            var bDigit = numB.size() > index ? numB.get(index) : 0;

            var newDigit = aDigit + bDigit + extra;

            extra = (int) newDigit / 2;
            result.add(newDigit % 2);

            index++;
            hasNext = numA.size() > index || numB.size() > index;
        }
    ...
    final static private int VERTEX_MAX = 100_001;
    List<Integer>[] vertexPairs = new List[VERTEX_MAX];
    boolean[] visitedVertices = new boolean[VERTEX_MAX];

    private void nextStep(String s, int vertex, List<Integer> indexes, List<Character> chars) {
        ...
        if (vertexPairs[vertex] != null) {
            for (var newVertex
                    : vertexPairs[vertex]) {
                nextStep(s, newVertex, indexes, chars);
            }
        }
    ...
    public String smallestStringWithSwaps(String s, List<List<Integer>> pairs) {
        ...
        for (var pair
             : pairs) {
            if (vertexPairs[pair.get(0)] == null) {
                vertexPairs[pair.get(0)] = new ArrayList<>();
            }
            if (vertexPairs[pair.get(1)] == null) {
                vertexPairs[pair.get(1)] = new ArrayList<>();
            }

            vertexPairs[pair.get(0)].add(pair.get(1));
            vertexPairs[pair.get(1)].add(pair.get(0));
        }

        final var result = s.toCharArray();
        for (var i = 0; i < s.length(); i++) {
            ...
            nextStep(s, i, indexes, chars);

            Collections.sort(indexes);
            Collections.sort(chars);
            
            for (var index = 0; index < chars.size(); index++) {
                result[indexes.get(index)] = chars.get(index);
            }
        }
    ...
    public String smallestStringWithSwaps1(String s, List<List<Integer>> pairs) {
        final var graphs = new ArrayList<Set<Integer>>();

        fillGraphs(graphs, pairs);

        final var stringArray = s.split("");
        final var usedChars = new int[stringArray.length];

        final var sortedStrings = new ArrayList<Queue<String>>();

        var currentGraphNum = 1;
        for (var graph
                : graphs) {
            final var sortedString = new PriorityQueue<String>(Comparator.comparing(o -> o));
            for (var index
                    : graph) {
                sortedString.add(stringArray[index]);

                usedChars[index] = currentGraphNum;
            }
        ...
        final var result = new StringBuilder(stringArray.length);

        for (var i = 0; i < stringArray.length; i++) {
            if (usedChars[i] == 0) {
                result.append(stringArray[i]);
            } else {
                result.append(sortedStrings.get(usedChars[i] - 1).poll());
            }
        }
        return result.toString();
    }

    private void fillGraphs(List<Set<Integer>> graphs, List<List<Integer>> pairs) {
        for (var pair
                : pairs) {
            Set<Integer> firstOccur = null;
            Set<Integer> secondOccur = null;

            for (var graph
                    : graphs) {
                if (graph.contains(pair.get(0))
                        || graph.contains(pair.get(1))) {
                    if (firstOccur == null) {
                        firstOccur = graph;
                    } else {
                        secondOccur = graph;
            ...
            if (firstOccur == null && secondOccur == null) {
                var newGraph = new TreeSet<Integer>();
                newGraph.add(pair.get(0));
                newGraph.add(pair.get(1));
                graphs.add(newGraph);
                continue;
            }

            if (firstOccur != null && secondOccur != null) {
                firstOccur.addAll(secondOccur);
                graphs.remove(secondOccur);
                continue;
            }

            if (firstOccur != null) {
                firstOccur.add(pair.get(0));
                firstOccur.add(pair.get(1));
            } else {
                secondOccur.add(pair.get(0));
                secondOccur.add(pair.get(1));
            }
        }
    }
    ...
    public int numberOfCombinations(String num) {
        ...
        var digits = Arrays.stream(num.split("")).mapToInt(Integer::parseInt).toArray();

        var threshold = 1_000_000_007;

        var count = 1;
        var currentNumber = digits[0];
        var current = 1;

        while (current < digits.length) {
            var tmp = current;
            while (tmp < digits.length) {
                var number = getNumber(digits, current, tmp);
    ...
    public int getNumber(int[] digits, int start, int end) {
        var result = 0;
    ...
    public int titleToNumber(String columnTitle) {
        final var map = new HashMap<String, Integer>(26);
        map.put("A", 1);
        map.put("B", 2);
        map.put("C", 3);
        map.put("D", 4);
        ...
        var size = columnTitle.length() - 1;

        var sum = 0;

        for(String digit
                : columnTitle.split("")) {
            sum += map.get(digit) * ((int) Math.pow(26, size));
            size--;
        }
    ...
    public int[] sortedSquares(int[] nums) {
        var queue = new PriorityQueue<Integer>(nums.length, Comparator.comparingInt(o -> o));

        for (var next
                : nums) {
            queue.add((int) Math.pow(next, 2));
        }
    ...
    public int minCostConnectPoints(int[][] points) {
        final var addedPoints = new boolean[points.length];
        final var leftPointsWithWeigh = new PriorityQueue<AbstractMap.SimpleImmutableEntry<Integer, Integer>>(
                Comparator.comparingInt(AbstractMap.SimpleImmutableEntry::getKey));

        var totalPath = 0;
        var countedPoints = 0;

        leftPointsWithWeigh.add(new AbstractMap.SimpleImmutableEntry(0, 0));

        while (countedPoints < points.length) {
            var newPointToAdd = leftPointsWithWeigh.poll();

            if (addedPoints[newPointToAdd.getValue()]) {
                continue;
            }

            totalPath += newPointToAdd.getKey();
            countedPoints++;
            addedPoints[newPointToAdd.getValue()] = true;
            ...
            for (var i = 0; i < points.length; i++) {
                ...
                leftPointsWithWeigh.add(
                        new AbstractMap.SimpleImmutableEntry<Integer, Integer>(
                                findPath(
                                        points[newPointToAdd.getValue()],
                                        points[i]),
                                i));
    ...
    public int minCostConnectPoints2(int[][] points) {
        ...
        var linkedTops = new ArrayList<int[]>();
        var allTops = new ArrayList<>(Arrays.stream(points).toList());

        var accumulatedPath = 0;

        //find core
        int[] firstTop = null, secondTop = null;
        var path = Integer.MAX_VALUE;
        for (var first
                : allTops) {
            for (var second
                    : allTops) {
                var curPath = findPath(first, second);
                if (curPath> 0
                        && curPath < path) {
                    path = curPath;
                    firstTop = first;
        ...
        linkedTops.add(firstTop);
        linkedTops.add(secondTop);

        allTops.remove(firstTop);
        allTops.remove(secondTop);

        accumulatedPath += path;

        //add new point one-by-one
        while (!allTops.isEmpty()) {
            ...
            for (var nextFree
                    : allTops) {
                var nextMin = findClosest(nextFree, linkedTops);
                var curPath = findPath(nextFree, nextMin);
            ...
            linkedTops.add(nextToAdd);
            allTops.remove(nextToAdd);
    ...
    private int[] findClosest(int[] top, List<int[]> points) {
        var closest = points.get(0);
        var closestPath = findPath(top, points.get(0));

        for (var current
                : points) {
            var path = findPath(top, current);
            if (path == 0) {
                continue;
            }
            if (closestPath > path) {
    ...
    public int minMeetingRooms(int[][] intervals) {
        ...
        final var roomQueue = new PriorityQueue<Integer>(
                intervals.length,
        ...
        for(int[] meeting
                : intervals) {
            if (roomQueue.size() > 0
                    && roomQueue.peek() <= meeting[0]) {
                roomQueue.poll();
            }
            roomQueue.add(meeting[1]);
        }
    ...
    public boolean canAttendMeetings(int[][] intervals) {
        var satOfMeetings = new HashSet<Meeting>();
        for(int[] meeting : intervals) {
            var curMet = new Meeting(meeting[0], meeting[1]);

            if (satOfMeetings.contains(curMet)) {
                return false;
            }

            satOfMeetings.add(curMet);
        }
    ...
    private int sumBetween(int[] nums, int start, int end) {
        var result = 0;
    ...
    private void recursion(TreeNode root, boolean isLeft) {
        if (root == null) return;
    ...
    public int[] intersection(int[] nums1, int[] nums2) {
        ...
        var result = new HashSet<Integer>(Arrays.stream(nums1).boxed().toList());
        var nums2List = new HashSet<Integer>(Arrays.stream(nums2).boxed().toList());
        result.retainAll(nums2List);


        var resultArray = new int [result.size()];
    ...
    public int searchInsert(int[] nums, int target) {
        ...
        while(true) {
            int getIndex = (startIndex + endIndex) / 2;
            if (nums[getIndex] == target) {
                return getIndex;
            }
    ...
    public int[] rearrangeArray(int[] nums) {
        ...
        var positive = new ArrayDeque<Integer>();
        var negative = new ArrayDeque<Integer>();
        ...
        for (var i = 0; i < nums.length; i++) {
            if (i % 2 == 0) {
                result[i] = positive.removeLast();
            } else {
                result[i] = negative.removeLast();
            }
        }
...
//    }
    ...
    public int reverse(int x) {
         ...
         var currentDigit = 0;
         var left = x;
         var listOfDigits = new ArrayDeque<Integer>();

         while (left != 0) {
             currentDigit = left % 10;
         ...
         var result = 0;
         var count = 0;
         while (!listOfDigits.isEmpty()) {
             int curDigit = listOfDigits.removeLast();

             if (count == 9) {
                if (curDigit < -2 || curDigit > 2) {
                    return 0;
                } else {
                    if (sign < 0) {
                        if (result < (Integer.MIN_VALUE % 1_000_000_000)) {
                            return 0;
                        }
                    } else {
                        if (result > (Integer.MAX_VALUE % 1_000_000_000)) {
                            return 0;
                        }
                    }
                }
             }
             result = result + (curDigit * ((int) Math.pow(10, count)));
    ...
    public String convert(String s, int numRows) {
        ...
        var arrayOfString = new String[numRows][s.length() / numRows];

        var x = 0;
        var y = 0;
        var way = 0; //0 - down, 1 - up

        for (var letter
                :s.split("") ) {
            arrayOfString[x][y] = letter;
        ...
        for (var xn = 0; xn < numRows; xn++) {
            for (var yn = 0; yn < s.length(); yn++) {
                if (arrayOfString[xn][yn] != null) {
                    result.append(arrayOfString[xn][yn]);
                }
            }
        }
        return result.toString();
    ...
    public int[] plusOne(int[] digits) {
        ...
        for (var i = digits.length -1; i >= 0; i--) {
            digits[i] = (digits.length -1 == i) ? digits[i] + 1 : digits[i] + addition;
            if (digits[i] == 10) {
                addition = 1;
                digits[i]=0;
            } else {
    ...
    public int removeDuplicates(int[] nums) {
        ...
        return nums.length - countOfDoubles;
    }
    ...
    public boolean isValid1(String s) {
        ...
        for (var symbol :s.split("")) {
            if (symbol.equals("}")
                ...
                var open =stack.removeLast();

                if ((open.equals("{") && symbol.equals("}"))
                        || (open.equals("[") && symbol.equals("]"))
                        || (open.equals("(") && symbol.equals(")"))) {
    ...
    public List<String> letterCombinations(String digits) {
        ...
        var letters = new ArrayList<List<String>>();
        letters.add(List.of(" "));
        letters.add(List.of());
        ...
        letters.add(List.of("p", "q", "r"));
        letters.add(List.of("t", "u", "v"));
        letters.add(List.of("w", "x", "y", "z"));

        if (digits.trim().length() == 1) {
            return letters.get(Integer.parseInt(digits.trim()));
        }

        var digitsArray = digits.trim().split("");
        var result = new ArrayList<String>();

        for (var letter
                : letters.get(Integer.parseInt(digitsArray[0]))) {
            result.add(letter);
        }

        digitsArray = digits.trim().substring(1).split("");
        ...
        for (var digit
            ...
            for (var letter
                    : letters.get(Integer.parseInt(digit))) {
    ...
    class Container {
        ...
        public boolean equals(Object o) {
            ...
            if (x == c.x) {
                if (y == c.y && z == c.z) return true;
                if (y == c.z && z == c.y) return true;
            }

            if (x == c.y) {
                if (y == c.x && z == c.z) return true;
                if (y == c.z && z == c.x) return true;
            }

            if (x == c.z) {
                if (y == c.x && z == c.y) return true;
                if (y == c.y && z == c.x) return true;
            }
    ...
    public List<List<Integer>> threeSum(int[] nums) {
        ...
        var allCombination = new HashSet<Container>();
        for (int x = 0; x < nums.length - 2; x++) {
            for (int y = x + 1; y < nums.length - 1; y++) {
                for (int z = y + 1; z < nums.length; z++) {
                    if ((nums[x] + nums[y] + nums[z]) == 0) {
                        var cur = new Container(nums[x], nums[y], nums[z]);
                        if (!allCombination.contains(cur)) {
                            allCombination.add(cur);
                        }
                    }
                }
            }
        ...
        return allCombination.stream().map(cur -> List.of(cur.x, cur.y, cur.z)).collect(Collectors.toList());
    }
    ...
    private void printTreeIteration(TreeNode root) {
    ...
    public void recoverTree(TreeNode root) {
        var listOfValue = new ArrayList<TreeNode>();
        allNodesHelper(root, listOfValue);

        findAndReplaceWrongSequence(listOfValue);

    }

    private void findAndReplaceWrongSequence(List<TreeNode> listOfValue) {
        TreeNode x = null;
        TreeNode y = null;
        var firstOccure = true;
        for (var i = 0; i < listOfValue.size() - 1; i++) {
            if (listOfValue.get(i).val > listOfValue.get(i + 1).val) {
                y = listOfValue.get(i + 1);
                if (firstOccure) {
                    x = listOfValue.get(i);
                    firstOccure = false;
                } else {
    ...
    private void allNodesHelper(TreeNode root, List<TreeNode> values) {
        if (root == null)
            return;

        allNodesHelper(root.left, values);
        values.add(root);
        allNodesHelper(root.right, values);
    }
    ...
    private List<Integer> getAllValues(TreeNode root) {
        ...
        if (root.left != null) {
            result.addAll(getAllValues(root.left));
        }

        result.add(root.val);

        if (root.right != null) {
            result.addAll(getAllValues(root.right));
        }
    ...
    private boolean correctHelper(TreeNode root) {
        if (root.left != null) {
            var allNodesLeft = getAllNodes(root.left);
            for (var node:
                    allNodesLeft) {
        ...
        if (root.right != null) {
            var allNodesLeft = getAllNodes(root.right);
            for (var node:
                    allNodesLeft) {
                if (node.val < root.val) {
                    var tmp = node.val;
    ...
    private List<TreeNode> getAllNodes(TreeNode root) {
        ...
        if (root.left != null) {
            result.addAll(getAllNodes(root.left));
        }

        result.add(root);

        if (root.right != null) {
            result.addAll(getAllNodes(root.right));
        }

        return result;
    }

    private boolean correctTree(TreeNode root) {
        var leftRoot = root;
        while (leftRoot.left != null) {
            leftRoot = leftRoot.left;
        }
        if (leftRoot.val > root.val) {
            var tmp = leftRoot.val;
        ...
        if (rightRoot.val < root.val) {
            var tmp = rightRoot.val;
            rightRoot.val = root.val;
            root.val = tmp;
            return true;
        }
    ...
    public ListNode addTwoNumbers(ListNode l1, ListNode l2) {
        ...
        while(true) {
            ...
            if (l1 != null && l2 != null) {
                if (result == null) {
                    result = new ListNode((l1.val + l2.val + addingSum) % 10);
                    current = result;
                } else {
                    current.next = new ListNode((l1.val + l2.val + addingSum) % 10);
                    current = current.next;
                }

                addingSum = (l1.val + l2.val + addingSum) / 10;
            ...
            if (l1 == null) {
                current.next = new ListNode((l2.val + addingSum) % 10);

                addingSum = (l2.val + addingSum) / 10;

                l2 = l2.next;
            } else {
                current.next = new ListNode((l1.val + addingSum) % 10);

                addingSum = (l1.val + addingSum) / 10;
    ...
    public int kthSmallest(TreeNode root, int k) {
        return getAllValues(root).stream().sorted().collect(Collectors.toList()).get(k - 1);
    }
    ...
    public TreeNode increasingBST(TreeNode root) {
        List<Integer> resultList = getAllValues(root);
        ListIterator<Integer> listIterator = resultList.listIterator(resultList.size());
        ...
        while (listIterator.hasPrevious()) {
            ...
            if (result == null) {
                ...
                result = new TreeNode(current, null, result);
            }
        }
...
//        D             500
//        M             1000
        final Map<String, Integer> mapRomanToArabic = new HashMap<>();
        ...
        final Map<String, String> subtractionMap = new HashMap<>();
        subtractionMap.put("I", "VX");
        subtractionMap.put("X", "LC");
        subtractionMap.put("C", "DM");
        ...
        for(String tmpChar : s.split("")) {
            ...
            if (currentChar.equals(tmpChar)) {
                currentCharCount++;
                continue;
            } else {

                int currentCharSum = mapRomanToArabic.get(currentChar) * currentCharCount;
                value += (subtractionMap.get(currentChar) != null
                        && subtractionMap.get(currentChar).contains(tmpChar)) ? currentCharSum * (-1) : currentCharSum;
        ...
        value += mapRomanToArabic.get(currentChar) * currentCharCount;
    ...
    public int[] twoSum(int[] nums, int target) {
        ...
        Map<Integer, Integer> mapOfResult = new HashMap<>();

        for(int i = 0; i < nums.length; i++) {
            if (mapOfResult.get(target - nums[i]) != null) {
                return new int[] {i, mapOfResult.get(target - nums[i])};
            } else {
                mapOfResult.put(nums[i], i);
            }
        }
        return null;
    }
